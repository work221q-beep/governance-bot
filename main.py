...
from db import init_indexes, server_configs, payload_armory
...
async def server_panel(request: Request, guild_id: str):
    ...
    if guild:
        for r in reversed(guild.roles): 
            ...
            roles.append({"id": str(r.id), "name": r.name, "color": str(r.color) if r.color.value != 0 else "#71717a", "current": current_perms, "is_everyone": r.name == "@everyone", "is_bot": r.managed})
...
async def sync_server_permissions(request: Request, guild_id: str):
    ...
    if guild:
        for r in guild.roles:
            ...
            try: await r.edit(permissions=discord.Permissions(**new_kwargs), reason="Sylas Live Web Sync")
...
async def permissions_manager(request: Request, guild_id: str):
    """NEW: The Advanced Permission Manager for 100k servers (Replaces Threat Map)."""
    user_cookie = request.cookies.get("session")
    if not user_cookie: return RedirectResponse("/")
    ...
    if guild:
        roles = [{"id": str(r.id), "name": r.name, "color": str(r.color) if r.color.value != 0 else "#71717a"} for r in reversed(guild.roles)]
        
        # Pull top 500 members to prevent RAM crashes on massive servers. The JS search will filter these.
    ...
    return templates.TemplateResponse("permissions.html", {"request": request, "guild_id": guild_id, "roles": roles, "users": users, "bots": bots, "guild_name": guild.name if guild else "Unknown"})
