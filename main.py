...
# SECURITY FIX: Handle HEAD requests to prevent 405 log exhaustion from uptime monitors
...
async def callback(request: Request, code: str = None, error: str = None, state: str = None):
    ...
    async with httpx.AsyncClient() as client:
        ...
        if not access_token:
            print(f"[OAuth Error] Failed to retrieve token: {token_data}")
...
async def apply_sync_post(request: Request, guild_id: str):
    ...
    for role in guild.roles:
        ...
        # SECURITY FIX: Enforce Hierarchy on Backend (Privilege Escalation Block)
        ...
        if role.permissions.value != new_perms.value:
            try: 
                ...
                await asyncio.sleep(0.3) # SECURITY FIX: API 429 Mitigation
...
async def chain2pay_webhook(request: Request):
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            # SECURITY FIX: Validate Amount matches invoice
            paid_amount = float(verify_data.get("amount", 0))
            expected_amount = float(payment.get("amount", 0))
            amount_valid = paid_amount >= expected_amount
            
            if verify_data.get("status") == "paid" and amount_valid:
                # SECURITY FIX: Atomic State Lock prevents Double-Spend
...
async def mod_action(request: Request, guild_id: str, action: str, target_id: str):
    ...
    # SECURITY FIX: Added Bounds Checking and robust Exception Handling (DoS patch)
    try:
        if action == "kick": 
...
async def channel_override(request: Request, guild_id: str, channel_id: str):
    ...
    # SECURITY FIX: Require Manage Channels AND Manage Roles to edit overrides
...
# SECURITY FIX: Replaced URL query-based auth with secure POST Auth
@app.get("/admin")
async def admin_panel(request: Request):
    ...
    # DB Bloat Fix: Purge expired pending transactions
    from db import payments
    yesterday = datetime.datetime.utcnow() - datetime.timedelta(hours=24)
    await payments.delete_many({"status": "pending", "created_at": {"$lt": yesterday}})
    
    # Admin Panel UI Fix: Strict 360 payload limit and filter out innocents
    raw_payloads = await payload_armory.find(
        {"raid_type": {"$not": re.compile(r"^innocent_")}}
    ).sort([("raid_type", 1), ("created_at", -1)]).to_list(360)
    ...
    for coll_name in collection_names:
        # SECURITY FIX: OOM Crash Vector Mitigation (limit DB pull to 50 instead of 1000)
...
async def admin_auth_post(request: Request):
    ...
    if hmac.compare_digest(key, ADMIN_KEY):
        ...
        # SECURITY FIX: Upgraded samesite to Strict to mitigate CSRF across all admin routes without modifying HTML
...
async def admin_edit_doc(request: Request, coll_name: str, doc_id: str):
    ...
    try:
        ...
        # SECURITY FIX: Reconstruct ISO strings back to Datetime to prevent TypeError crashes
