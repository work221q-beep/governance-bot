...
# GLOBAL HTTP CLIENT: Keeps the connection "warm" to eliminate TCP handshake latency
...
def get_ai_prompt(raid_type: str) -> str:
    ...
    # BATCHING: Generating 5 at a time reduces API calls and bypasses rate limits faster
    ...
    if raid_type == "phishing":
        return base_req + f"[Seed: {seed}] Generate 5 realistic Discord phishing links (e.g., fake steam, fake nitro, obfuscated domains)."
    elif raid_type == "spam_flood":
        return base_req + f"[Seed: {seed}] Generate 5 highly repetitive, annoying bot spam messages (e.g., crypto, free gifts, discord.gift/xyz)."
    elif raid_type == "fake_mod":
        return base_req + f"[Seed: {seed}] Generate 5 messages where a user uses social engineering to pretend to be a server admin/mod demanding permissions or user data."
    elif raid_type == "insider_threat":
        return base_req + f"[Seed: {seed}] Generate 5 messages where a previously trusted user suddenly goes rogue (e.g., posting fake malicious server announcements or spreading severe misinformation)."
    elif raid_type == "escalation":
        return base_req + f"[Seed: {seed}] Generate 5 highly toxic messages showing a slow escalation into severe harassment/threats over a gaming argument."
    elif raid_type == "harassment":
        return base_req + f"[Seed: {seed}] Generate 5 messages where a coordinated group is viciously brigading/targeting a single specific user in the chat."
    
    # CONTEXTUAL INNOCENT PROMPTS (False Positives)
    elif raid_type == "innocent_phishing":
        return base_req + f"[Seed: {seed}] Generate 5 completely SAFE, legitimate links shared casually between gamers (e.g., a real YouTube video, a real Steam store page, a Wikipedia link)."
    elif raid_type == "innocent_spam_flood":
        return base_req + f"[Seed: {seed}] Generate 5 harmless messages of a user typing fast/excitedly in all caps about a game update (Looks like spam, but is innocent)."
    elif raid_type == "innocent_fake_mod":
        return base_req + f"[Seed: {seed}] Generate 5 normal messages of regular users asking legitimate questions to the moderation team about server rules."
    elif raid_type == "innocent_insider_threat":
        return base_req + f"[Seed: {seed}] Generate 5 normal, helpful messages from long-time trusted members answering newbie questions."
    elif raid_type == "innocent_escalation":
        return base_req + f"[Seed: {seed}] Generate 5 messages showing a passionate, but entirely respectful and rule-abiding debate about game balance/meta."
    elif raid_type == "innocent_harassment":
        return base_req + f"[Seed: {seed}] Generate 5 messages showing friendly, harmless banter/teasing between close friends in a guild."

    return base_req + "Generate 5 casual gaming chat messages."
...
async def harvest_payloads(raid_type: str):
    ...
    try:
        # CONTROLLED FALLBACKS: Prioritize Nvidia -> Trinity -> Stepfun
